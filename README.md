...
För varje DB skapade en *package* i SSIS med sina respektiva *dataflow*.  
...
Arkivet består av sex stora filer utan format som fungerar som csv-filer och motsvarar  åren 2003-2008 med ungefar 42 milljoner rader. Det finns också tre små csv-filer som behåller information om flygplatser, flygplan och carrier.


I varje *package* börjar jag med en *Truncate table* för att undvika dubbletter. Efter det skapade jag en *ForeachLoop* som anropar tabeller med åren. I den här delen använder jag *suggest type* för att räkna och bestämma vilken *datatype* jag ska  använda. Totalt undersökes 200,000 rader för att nå lite mer noggrannhet.  
Efter det körde jag samtidigt de andra tabellerna för att effektivisera processen. Jag gjorde en extra process i Data-plane för att den tabellen hade citattecken i några kolumner medan andra inte hade det. Då behövde jag hantera *Flat File Connection Manager Editor* *column delimiter: Mixed.* Men den räckte inte och för att testa skapades en *flat file* med de som inte följde strukturen genom att *redirect row*. Sedan valdes de igen  och jag fixade datatyper och raderade citattecken om dem forfarande fanns kvar. Likadan process gjorde jag med Carrier. Obs! Man behöver inte skapa en till .txt file, men jag ville prova det. 
...
Jag ritade några boxar för att tydliggöra min process och jag försökte att i varje Package hålla en struktur med nivåer för att ha lite ordning. Grön box är transformationer, blå box är conditional split och lila är ”NA”- tabell. 

Jag igångsatte transformationen med att kontrollera *CancellationCode* i *Derived Column*. Först nu kontrollerades alla null-värde och konverterades till ”N” och sen kontrollerades alla ”NA” till ”N”(Jag kunde göra det i samma element men jag ville göra så för att kunna använda *Data viewer* och kolla att allt stämde)  Efter det använde jag *conditional split* för att filtrera, de som hade Null eller NA skulle gå till ”NA-Tabell” (lila boxen). 
